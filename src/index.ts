...
import { compressMultipleVideos } from "./compress";
...
program
  .argument(
    "[files...]",
    "Video files to compress (multiple allowed, wildcards supported)",
  )
  ...
  .action(async (files: string[], options) => {
    try {
      ...
      if (!VALID_QUALITIES.includes(options.quality)) {
        console.error(
          `❌ Error: Invalid quality "${options.quality}". Must be: high, medium, or low`,
        );
        process.exit(1);
      }
